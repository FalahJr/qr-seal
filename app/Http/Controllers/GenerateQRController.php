...
class GenerateQRController extends Controller
{
    ...
    public function create(StoreGenerateQRRequest $request)
    {
        ...
        try {
            ...
            // Buat QR code
            $isiQQR = url('validasi/' . $model->id . '?rsa=' . $model->digital_code);
            $qrCode = new QrCode($isiQQR);
            // $qrCode->setSize(5000);
            // $qrCode->setMargin(1000);
            $qrCode->setSize(300); // Mengurangi ukuran QR code
            $qrCode->setMargin(10); // Mengurangi margin QR code
